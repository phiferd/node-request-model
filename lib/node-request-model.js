...
function getParams (req, definition) {
  ...
  Object.keys(definition).forEach(key => {
    const paramDef = typeof definition[key] === 'string'
      ...
      : definition[key];
    output[key] = getParam(req, key, paramDef);
    if (paramDef.required && typeof output[key] === 'undefined') {
      const paramName = paramDef.name || key;
      throw Error(`${paramName} is a required parameter`);
    }
    if (paramDef.enum && !paramDef.enum.includes(output[key])) {
      const paramName = paramDef.name || key;
      throw Error(`${paramName} was ${output[key]}. Must be one of: ${paramDef.enum.join(', ')}`);
    }
  })
  return output;
}
